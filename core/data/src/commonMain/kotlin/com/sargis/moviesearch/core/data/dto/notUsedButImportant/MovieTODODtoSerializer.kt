...
class MovieTODODtoSerializer : KSerializer<MovieTODODto> {

    override val descriptor: SerialDescriptor
        get() = buildClassSerialDescriptor(
            MovieTODODto::class.simpleName!!
        ) {
    ...
    override fun deserialize(decoder: Decoder): MovieTODODto = decoder.decodeStructure(descriptor) {
            ...
            when (val index = decodeElementIndex(descriptor)) {
                0 -> {
                    ...
                    description = if (element is JsonObject) {
                        decoder.json.decodeFromJsonElement<TODODescriptionDto>(
                            element = element,
                            deserializer = TODODescriptionDto.serializer()
                        ).value
        ...
        return@decodeStructure MovieTODODto(
